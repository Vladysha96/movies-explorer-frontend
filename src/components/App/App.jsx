...
function App() {
  ...
  const handleLogin = (user) => {
    ...
    mainApi
      ...
      .catch(() => {
        showAlert(ALERT_MESSAGES.SUCCESSFULLY.ERROR.AUTH);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const getUserData = (jwt) => {
    mainApi
      ...
      .catch(() => {
        clearLocal();
        showAlert(ALERT_MESSAGES.SUCCESSFULLY.ERROR.GET_USER);
        throw new Error();
      })
      .finally(() => {
        setPreloader(false);
  ...
  const handleUpdateUser = (user) =>
    mainApi
      ...
      .catch(() => {
        showAlert(ALERT_MESSAGES.SUCCESSFULLY.ERROR.UPDATE_PROFILE);
        throw new Error();
      });
  ...
  const handleLikeMovieClick = (movieId, movie) =>
    movieId
      ? mainApi.deleteLikeMovie(movieId, token).catch(() => {
        showAlert(ALERT_MESSAGES.SUCCESSFULLY.ERROR.DELETE_FILM);
        throw new Error();
      })
      : mainApi.addLikeMovie(movie, token).catch(() => {
        showAlert(ALERT_MESSAGES.SUCCESSFULLY.ERROR.ADD_FILM);
        throw new Error();
      });
